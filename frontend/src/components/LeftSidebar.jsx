    ...
    text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
        <div className='pb-5'>
            ...
            <div className='bg-[#282141] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
                ...
                <input type="text" placeholder='Search users' className='bg-transparent 
                border-none outline-none text-white text-xs placeholder:text-[#c8c8c8] flex-1' />
            </div>
        </div>

        <div className='flex flex-col'>
            {userDummyData.map((user , index) => (
                <div key={user._id} onClick={() => setSelectedUser(user)} className={`relative flex 
                items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id ? 'bg-[#282141]' : 'hover:bg-[#282141]/50'}`}>
                    <img src={user.profilePic || assets.avatar_icon} alt={user.fullName} className='w-8.75 aspect-square rounded-full' />
